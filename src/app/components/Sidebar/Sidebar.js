...
// Sidebar component
const Sidebar = ({ theme, currentLanguage, onLanguageChange }) => {
  ...
  return (
    <div className={`w-12 ${sidebarBg} border-r ${sidebarBorder} flex flex-col items-center py-4 relative z-10 shadow-lg`}> {/* Changed width to w-12 */}
      {/* Top section of sidebar icons (Main Tabs) */}
      <div className="flex flex-col space-y-2 flex-grow"> {/* Changed space-y-3 to space-y-2 */}
        <SidebarIcon
          icon={Home}
          tooltip="Home"
          onClick={() => handleNavigation('/')}
          isActive={pathname === '/' && currentLanguage === null}
        />
        <SidebarIcon
          icon={Search}
          tooltip="Search"
      ...
      <div className="flex flex-col space-y-2 mt-8 mb-4 px-1"> {/* Changed space-y-3 to space-y-2 */}
        <SidebarIcon
          icon={SiJavascript}
          tooltip="JavaScript"
